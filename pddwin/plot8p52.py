...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.btnAdd.clicked.connect(self.plot_) # 绑定添加按钮的单击信号
    ...
    def getItem(self, item):
        if item.column() == 0:  # 如果单击的是第一列
            ...
            # 根据用户名称查询用户信息
            result = service.query("select * from tb_user where userName=%s",item.text())
            self.editPwd.setText(result[0][1]) # 显示用户密码
